<?php
	function createClassicDiceRoll($server_seed, $client_seed)
	{
		$hash = hash("sha512", $server_seed);
		$hash = hash_hmac("sha512", $client_seed, $hash);
		$sub_hash = substr($hash, 0, 8);
		$decimal = hexdec($sub_hash);
		$roll = ($decimal / 4294967296 * 10001 / 100);
...
<html lang="en">
  ...
  <body>
    <div id="app" class="main">
      ...
      <hr />
      <form class="py-5" method="POST" action="classicdice.php">
	  ...
	  <?php
			if (isset($_POST["server_seed"]) and !empty($_POST["server_seed"]) and isset($_POST["client_seed"]) and !empty($_POST["client_seed"]))
			{
				...
				$result = createClassicDiceRoll($server_seed, $client_seed);
				$hash = $result['hash'];
				$roll = $result['roll'];
				echo '<hr />';
